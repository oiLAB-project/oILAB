 ...
 * */
template <int dim> class IntegerLattice {
...
public:
  ...
  perpendicularDirections(const Eigen::Vector<IntScalarType, dim> &in) {
    Eigen::Matrix<IntScalarType, dim - 1, dim> out =
        Eigen::Matrix<IntScalarType, dim - 1, dim>::Zero();
    ...
    if (firstNonZero == dim - 1)
      return out;

    out(firstNonZero, firstNonZero) =
        -in(firstNonZero + 1) /
        IntegerMath<IntScalarType>::gcd(in(firstNonZero), in(firstNonZero + 1));
    out(firstNonZero, firstNonZero + 1) =
        in(firstNonZero) /
        IntegerMath<IntScalarType>::gcd(in(firstNonZero), in(firstNonZero + 1));
    ...
    for (int ind = firstNonZero + 2; ind < dim; ind++) {
      ...
      // truncated_in=
      // truncated_in/IntegerMath<IntScalarType>::gcd(truncated_in);

      IntScalarType x, y;
      DiophantineSolver<IntScalarType>::solveDiophantine2vars(
          truncated_in(0), truncated_in(1),
          -truncated_in(2) *
              IntegerMath<IntScalarType>::gcd(truncated_in(0), truncated_in(1)),
          x, y);
      ...
      out.row(ind - 1) =
          out.row(ind - 1) / IntegerMath<IntScalarType>::gcd(out.row(ind - 1));
    }
    return out;
  }
    };
