...
template <typename StateType, typename SystemType, typename EnsembleType,
          typename EvolveType>
MonteCarlo<StateType, SystemType, EnsembleType, EvolveType>::MonteCarlo(
    const EnsembleType &ensemble, const EvolveType &evolve)
    : EvolutionAlgorithm<StateType, SystemType, EvolveType>(evolve),
      ensemble(ensemble),
...
template <typename StateType, typename SystemType, typename EnsembleType,
          typename EvolveType>
void MonteCarlo<StateType, SystemType, EnsembleType, EvolveType>::evolve(
    const int &maxIterations) {
  int acceptCount = 0;

  for (int i = 0; i < maxIterations; ++i) {
    auto proposedState = ensemble.sampleNewState(currentState, false);
    bool transition = this->acceptMove(
        std::make_pair(proposedState,
                       ensemble.constructMesoState(proposedState)),
        std::make_pair(currentState,
                       ensemble.constructMesoState(currentState)));
    if (transition) {
