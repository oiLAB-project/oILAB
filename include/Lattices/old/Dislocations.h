...
class Dislocations : private std::vector<DislocationDipole> {
  ...
  double line_search(const Vector2d &x, const Vector2d &dx, const Vector2d &y,
...
public:
  ...
  double edgeElasticEnergyKernel(const Vector2d &x1, const Vector2d &x2,
                                 const Vector2d &b1, const Vector2d &b2) const;
